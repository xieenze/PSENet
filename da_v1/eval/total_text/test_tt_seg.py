...
def solverange(img_id, func_first, func_second,img_name):
    max_size = 1500  # config.eval_image_max_size
    short_size = 848 # config.eval_image_short_size
    nms_thres = 0.3
    ...
    for c in range(2):
        ...
        if tmp == []:
            ...
            seg_score = res[-1]
            return None,inp,img_id,seg_score,sz
    ...
    # **** Add for semantic segmentation
    seg_score = res2[4]
    """
    pred = seg_score[0].argmax(axis=0)
    pred[pred == 2] = 0
    pred = cv2.resize(pred.astype(np.uint8), (sz[1], sz[0]))
    """
    # **** End
    all_info = []
    ...
    return all_info,inp,img_id,seg_score,sz
...
def draw_seg(seg_score, img_name, sz):
    pred = seg_score[0].argmax(axis=0)
    pred = cv2.resize(pred.astype(np.uint8), (sz[1], sz[0]))
    pred = pred * 255
    save_path = './output/out_seg/' + img_name + '.jpg'
    cv2.imwrite(save_path, pred)
    print('Save the image ' + img_name)
...
def test(p_id,epoch,offset):
    ...
    for img_id in range(p_id+offset, 1880, num_gpu):
        ...
        all_info, img, img_id, score, sz = solverange(img_id, m1, m2, img_name)
        write_res(all_info, img, img_id)
        if not (score is None):
            draw_seg(score, str(img_id), sz)
        masks = []
