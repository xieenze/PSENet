...
def train(train_loader, model, model_frozen, criterion, optimizer, epoch):
    model.train()
    ...
    for batch_idx, (imgs, gt_texts, gt_kernels, training_masks, tt_imgs) in enumerate(train_loader):
        ...
        outputs = model(imgs)
        blocks_source = model(tt_imgs,from_tt=True)
        blocks_target = model_frozen(tt_imgs)

        loss_domain = L2_loss(blocks_source[0], blocks_target[0])
        ...
        if batch_idx % 20 == 0:
            ...
            output_log  = '({batch}/{size}) Batch: {bt:.3f}s | TOTAL: {total:.0f}min | ETA: {eta:.0f}min | Loss: {loss:.4f} | Loss_K: {loss_kernel:.4f} | Loss_T: {loss_text:.4f} | Loss_D: {loss_domain:.4f} | Acc_t: {acc: .4f} | IOU_t: {iou_t: .4f} | IOU_k: {iou_k: .4f}'.format(
                batch=batch_idx + 1,
...
def main(args):
    ...
    if args.arch == "resnet50":
        ...
        model_frozen = models.resnet50_frozen(pretrained=True)
    ...
    model_frozen = torch.nn.DataParallel(model_frozen).cuda()
